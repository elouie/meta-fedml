...
def run_global_emnist_experiment(lr=0.001, batch=32, epochs=10):
    train_dataset, val_dataset = load_flattened_dataset(batch)
    step_count = ceil(train_dataset[1] / batch)
    val_step_count = ceil(val_dataset[1] / batch)

    # Callbacks during execution
    checkpoint_path = "global-weights-ep{epoch:02d}-acc{val_accuracy:.2f}.hdf5"
    ...
    model.compile(optimizer=opt,
                  ...
                  metrics=['accuracy'])
    history = model.fit(train_dataset[0].repeat(epochs),
